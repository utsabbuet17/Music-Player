...
status_Bar=ttk.Label(root,text='Welcome to Music',relief = SUNKEN, anchor=E,font = 'Arial 15 italic')
...
def about_us():
    tkinter.messagebox.showinfo('About Music','This is a Audio Player created using Pyhton3 tkinter by Utsab084')
...
addbtn = ttk.Button(leftframe,text="Add",command=file_browse)
...
filelabel1=Label(leftframe,text = 'Playlist',font = 'Times 15 italic',fg='blue')
filelabel1.pack()

topframe = Frame(rightframe)
topframe.pack(pady=30)

file_length = ttk.Label(topframe,text = 'Duration - 00:00',relief = GROOVE, font = 'Arial 15 roman')
file_length.grid(row=0,column=0,padx=10)

currenttimelabel = ttk.Label(topframe,text = 'Current Time - 00:00', relief = GROOVE, font = 'Arial 15 roman')
currenttimelabel.grid(row=0,column=1,padx=10)
...
rewindButton=ttk.Button(bottom_frame, image=rewindImage, command=rewind_sangeet)
#stopButton.pack(side=LEFT,padx=10)
...
speakerButton=ttk.Button(bottom_frame, image=speakerImage, command=mute_sangeet)
speakerButton.grid(row=0,column=2,padx=10,pady=10)


scale=ttk.Scale(bottom_frame,from_=0,to=100, orient=HORIZONTAL, command=cng_vol)
scale.set(40)
mixer.music.set_volume(0.4)
